...
db<-read.all.processed(list_files, ts_start, ts_end)
str(db)
```
...
db.qi.2y <- summarySE(db.qi.2y, measurevar = "value", groupvars = c("ts", "class"))
```


```{r}
plot_qi_2_year<-
  ggplot(data = db.qi.2y,  aes(x=ts, y=value))+
