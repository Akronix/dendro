...
```{r}
if (length(selected_dendros) > 1) {db = db %>% filter(db$series %in% selected_dendros)}
str(db)
```
...
```{r}
seasons.qi.allperiod = seasonalities.qi.agg %>% mutate(time_of_day = as_hms(ts)) %>% summarise(mean_time_day = mean(mean), SE_time_day = sd(sd)/sqrt(n()), .by = time_of_day)
seasons.qi.allperiod

ggplot( data = seasons.qi.allperiod, mapping = aes(x=time_of_day, y = mean_time_day)) + 
  geom_line() + 
  geom_line(aes (x = time_of_day, y = (mean_time_day + SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
  geom_line(aes (x = time_of_day, y = (mean_time_day - SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
  ggtitle("Aggregated mean in one day of seasonalities for Quercus Ilex") +
...
```{r}
seasons.qi.jun_jul = seasonalities.qi.agg %>% filter( (ts >= as.Date("2023-06-01") ) & (ts < as.Date("2023-08-01")) ) %>%  mutate(time_of_day = as_hms(ts)) %>% summarise(mean_time_day = mean(mean), SE_time_day = sd(sd)/sqrt(n()), .by = time_of_day)
seasons.qi.jun_jul

ggplot( data = seasons.qi.jun_jul, mapping = aes(x = time_of_day, y = mean_time_day)) + 
  geom_line() + 
  geom_line(aes (x = time_of_day, y = (mean_time_day + SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
  geom_line(aes (x = time_of_day, y = (mean_time_day - SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
  ggtitle("Aggregated mean in one day of seasonalities from June to July 2023 for Quercus Ilex") +
  scale_x_time(breaks = seq(0, 85500, by = 3600), labels = every_hour_labels) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Hour of the day", y = "Micrometers of Daily seasonality (um)")
...
ggplot(seasons.periods.joined) +
  ...
  geom_line(aes(x = time_of_day, y = mean_time_day, col = period)) + 
  geom_line(aes (x = time_of_day, y = (mean_time_day + SE_time_day), col = period), alpha = 0.5, linetype = "dashed", show.legend = F) +
  geom_line(aes (x = time_of_day, y = (mean_time_day - SE_time_day), col = period), alpha = 0.5, linetype = "dashed", show.legend = F) +
  scale_x_time(breaks = seq(0, 85500, by = 3600), labels = every_hour_labels) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Hour of the day", y = "Micrometers of Daily seasonality (um)")
...
# ggplot() +
#   ggtitle("Aggregated mean in one day for all period And for period june-july") +
#   geom_line(data = seasons.qi.allperiod, mapping = aes(x=time_of_day, y = mean_time_day), col = "blue") + 
#   geom_line(data = seasons.qi.allperiod, aes (x = time_of_day, y = (mean_time_day + SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
#   geom_line(data = seasons.qi.allperiod, aes (x = time_of_day, y = (mean_time_day - SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
#   geom_line(data = seasons.qi.jun_jul, mapping = aes(x = time_of_day, y = mean_time_day), col= "purple") +
#   geom_line(data = seasons.qi.jun_jul, aes (x = time_of_day, y = (mean_time_day + SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
#   geom_line(data = seasons.qi.jun_jul, aes (x = time_of_day, y = (mean_time_day - SE_time_day)), col = "red", alpha = 0.5, linetype = "dashed", show.legend = F) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
#   scale_x_time(breaks = seq(0, 85500, by = 3600), labels = every_hour_labels) +
#   labs(x = "Hour of the day", y = "Micrometers of Daily seasonality (um)")
...
plot_temp_and_humidity
#ggsave('output/VWCytempAgg-selected-period-Miedes.png', width = 15, height = 10)
```
...
ggplot( data = seasons.qi.jun_jul, mapping = aes(x=time_of_day, y = mean_time_day)) + 
  ...
  geom_line(aes (x = time_of_day, y = (mean_time_day + SE_time_day)), col = season.color, alpha = 0.5, linetype = "dashed", show.legend = F) +
  geom_line(aes (x = time_of_day, y = (mean_time_day - SE_time_day)), col = season.color, alpha = 0.5, linetype = "dashed", show.legend = F) +
  ggtitle("Aggregated mean in one day of seasonalities plus daily temperature variation from June to July 2023 for Quercus Ilex") +
  scale_x_time(breaks = seq(0, 85500, by = 3600), labels = every_hour_labels ) +
  labs(x = "Hour of the day", y = "Micrometers of Daily seasonality (um)") +
  scale_y_continuous(breaks = seq(-5,5,1), sec.axis = sec_axis(trans = ~ ((. * 5) + 23), name = "Temperature (ºC)", breaks = seq(0,40,5)) ) +
  geom_line(data = temp.jun_jul, aes(x = timeOfDay, y = (meanTemp - 23) / 5, col = "Temperature"), alpha = 0.6, show.legend = T) +
  geom_line(data = temp.jun_jul, aes(x = timeOfDay, y = (meanTemp - 23) / 5 + se_temp), col = temperature.color, alpha = 0.4, linetype = "dashed") +
  geom_line(data = temp.jun_jul, aes(x = timeOfDay, y = (meanTemp - 23) / 5 - se_temp), col = temperature.color, alpha = 0.4, linetype = "dashed") +
