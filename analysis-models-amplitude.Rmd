...
hist(amp_miedes2$mean.temp)
hist(amp_miedes2$max.temp)
hist(amp_miedes2$range.temp)
hist(amp_miedes2$mean.vwc)
hist(amp_miedes2$max.vwc)
hist(amp_miedes2$range.vwc)
```
...
hist(amp_miedes2$ampl)
hist(log(amp_miedes2$ampl+0.1))
```

```{r}
model.glm <- glm(log(ampl+0.1) ~ 1 + id + doy, data = ampl.df, family = "gaussian")
```


```{r}
library(lme4)
model.lme4 <- lmer(log(ampl+0.1) ~ 1 + (1|id), data = ampl.df)
#model <- lmer(log(ampl+0.1) ~ 1 + (1|class) + (1|id) + (1|doy) + (1|site), data = ampl.df)
#model <- lmer(log(ampl+0.1) ~ 1, random = ~ 1 + class + id + doy + site, data = ampl.df)
#model.clim <- lmer(log(ampl) ~ max.vwc * range.temp + (1|class) + (1|id) + (1|doy) + (1|site) , data = ampl.df)
```

Now using nlme
```{r}
library(nlme)
model.lme <- lme(log(ampl+0.1) ~ 1, random = ~ 1|id, data = ampl.df)
model.lme.autocor <- update(model.lme, correlation=corAR1())
plot(ACF(model.lme, alpha=0.05))
...
```{r}
library(glmmTMB)
model.glmmTMB <- glmmTMB(log(ampl+0.1) ~ 1 + (1|id) + (1|doy), data=ampl.df, family=gaussian) # add up 
model.glmmTMB.cor <- glmmTMB(log(ampl+0.1) ~ 1 + (1|id)  + (1|doy) + ar1(as.factor(doy)-1|id), data=ampl.df, family=gaussian) # add up temporal autocorrelation
model.glmmTMB2 <- glmmTMB(log(ampl+0.1) ~ class + site + (1|id) + (1|doy), data=ampl.df, family=gaussian)
...
```{r}
AICtab(model.glmmTMB, model.glmmTMB.cor, model.glmmTMB2, model.glmmTMB2.cor, model.lme4, model.glm, model.lme, model.lme.autocor, model.glmmTMB2.climate, weights=TRUE, delta = T, base = T)
```
