...
plot_cat_full_year<-
  ...
  ggtitle("Mean of data for dendrometers with ± standard deviation strips") +
...
#   ggplot(data = db.by.class, mapping = aes(x=ts, y=value, col = class))+
#   geom_line( ) +
#   # ggtitle(paste0("Dendro data for sensor series: ",db$series[1], " - ", db$sp[1])) +
#   labs(x=expression('Date'),
#        y=expression( Delta*"D (um)") ) +
...
```{r}
plot_multiple_dendro(db.Qi, "Quercus Ilex", y = db.Qi$value)
ggsave('output/Qi-Corbalan.png', width = 15, height = 10)
plot_multiple_dendro(db.ND, "Not Declined", y = db.ND$value)
ggsave('output/ND-Corbalan.png', width = 15, height = 10)
plot_multiple_dendro(db.D, "Declined", y = db.D$value)
...
```{r}
plot_mean_sd(db.Qi.agg, "Quercus Ilex", output_fn = "output/mean-Qi.png")
```
...
```{r}
plot_mean_sd(db.D.agg, "Declined Pinus", output_fn = "output/mean-declined.png" )
```
...
```{r}
plot_mean_sd(db.ND.agg, "Not Declined Pinus", output_fn = "output/mean-ND.png" )
```
...
```{r}
# plotByClass <- 
#   ggplot(data = db.by.class,  aes(x=ts, y=mean, col=class)) +
#     ggtitle("Means of data by class with ± standard deviation strips") +
#     geom_line() +
#     geom_ribbon(aes(ymin=mean-sd, ymax=mean+sd, fill=class), alpha=0.2, show.legend	= FALSE, linetype = 0) +
...
```{r}
plot_multiple_dendro(db.env, "Temperature per TMS sensor", y = db.env$temp)
```
...
```{r}
plot_multiple_dendro(db.env, "Volumetric Water Content per TMS sensor", y = db.env$vwc)
ggsave('output/vwc-Corbalan.png', width = 15, height = 10)
```
