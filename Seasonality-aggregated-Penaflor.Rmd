...
selected_dendros <- FALSE # except for 92222178 which is excluded
```
...
Mean seasonality for Quercus Coccifera:
```{r}
...
ggplot( data = seasonalities.qi, mapping = aes(x=ts, y = value, color = series)) + 
  geom_line(alpha = 0.5) +
  ggtitle(glue('Quercus Coccifera seasonalities by dendrometer - {PLACE} '))
...
ggplot( data = seasonalities.qi.agg, mapping = aes(x=ts, y = mean)) + 
  geom_line() + 
  ggtitle("Mean of seasonalities for Quercus Coccifera")
...
With all daily amplitudes, calculate mean of each one by class and for all trees
...
plot_temp_and_humidity <-
  ggplot(data = db.env, aes(x=ts, y=surface.temp)) +
    ggtitle(glue("Temperature and humidity for {PLACE} (2022-2023)")) +
...
plot_temp_and_humidity
#ggsave('output/VWCytempAgg-selected-period-Penaflor.png', width = 15, height = 10)
```
...
We found that after 6 instances of 15 mins (1h and half), there is 0.70 inverse correlation within surface temperature and Quercus Coccifera maximum swelling.
