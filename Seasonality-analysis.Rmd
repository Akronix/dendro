...
```{r}
db.env.agg <- read.env.agg(file.path(PATH,ENV_DIR,'aggregated','proc-agg-env.csv'))
```

Filter env data to period of study
```{r}
db.env.agg <- db.env.agg[which(db.env.agg$ts>=ts_start & db.env.agg$ts<=ts_end),]
summary(db.env.agg)
```
...
```{r}
clim.daily <- db.env.agg %>% mutate (date = date(ts)) %>% group_by(date) %>% summarise(max.temp = max(soil.temp), min.temp = min(soil.temp), mean.temp = mean(soil.temp), sd.temp = sd(soil.temp), mean.vwc = mean(vwc), sd.vwc = sd(vwc))
summary(clim.daily)
```
...
cor.test(
  ...
  clim.daily$mean.vwc, amplitude.df$mean,
...
cor.test(
  #specify the two variables to correlate
  clim.daily$max.temp, amplitude.df$mean,
  # correlation methods (pearson, spearman, kendall)
  method = c("pearson"),
  # set confidence interval
  conf.level = 0.95)
...
```{r}
ccf (clim.daily$mean.vwc, amplitude.df$mean,
...
find_Max_CCF(clim.daily$mean.vwc, amplitude.df$mean)
```
