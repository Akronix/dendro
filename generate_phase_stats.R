...
phase_stats_data <- phase_stats(dendro_L2 = db, plot_phase = F, plot_export = F, tz = 'Madrid/Spain', agg_daily = F)

print(phase_stats_data)

summary(phase_stats_data)

write_csv(phase_stats_data, file.path(OUTPUT_PATH, glue('phase_stats-{PLACE}.csv')), append = F, col_names = T)
