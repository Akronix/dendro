...
```{r}
db.gr2022 <- db.gr2022 %>% summarise( mean_gr = mean(rate, na.rm = T), gr_se = sd(rate / sqrt(n()), na.rm = T), .by = c(date, class)) %>% mutate(doy = yday(date))
db.gr2023 <- db.gr2023 %>% summarise( mean_gr = mean(rate, na.rm = T), gr_se = sd(rate / sqrt(n()), na.rm = T), .by = c(date, class)) %>% mutate(doy = yday(date))
```
...
```{r}
if (SAVE) ggsave(glue('output/GrowthRate-byClass-{PLACE}.svg'), combined, width = 4200, height = 3000, units = "px")
```
