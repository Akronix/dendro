...
ggplot(data = amplitude.df, mapping = aes(x = date, y = mean, col = class)) +
  ...
  geom_line(data = clim.daily, mapping = aes(x = date, y = range.vwc * 100, linetype = "Max Volumetric Water Content (%)"), col="blue")
```
...
cor.test(
  #specify the two variables to correlate
  clim.daily$interquartil.temp, amplitude.P_D$mean,
  # correlation methods (pearson, spearman, kendall)
  method = c("pearson"),
  # set confidence interval
  conf.level = 0.95)
...
```{r}
ccf (clim.daily$max.temp, clim.daily$range.temp,
...
```{r}
find_Max_CCF(amplitude.P_ND$mean, clim.daily$interquartil.temp)
```
...
join.df <- full_join(clim.daily, amplitude.P_ND, by="date")
  ggplot(data = join.df, aes(log10(mean.vwc), log10(mean))) +
  # black points graph
  geom_point() +     
...
```{r}
plot(log10(clim.daily$mean.vwc), log10(amplitude.P_ND$mean))
