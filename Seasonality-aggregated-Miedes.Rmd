...
Select dendros for this analysis (all if commented)
```{r}
...
```{r}
if (exists('selected_dendros')) {db = db %>% filter(db$series %in% selected_dendros)}
str(db)
```
...
ggsave(glue('output/{PLACE}-mean-seasonalities.png'), width = 15, height = 10)
```
